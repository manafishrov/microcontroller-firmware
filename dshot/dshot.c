...
static void dshot_sm_config_set_pin(struct dshot_controller *controller,
                                    int pin) {
...
void dshot_controller_init(struct dshot_controller *controller,
  ...
  float clkdiv =
      (float)clock_get_hz(clk_sys) / (1000.0f * (float)dshot_speed * 40.0f);
...
static void dshot_interpret_erpm_telemetry(struct dshot_controller *controller,
                                           uint16_t edt) {
  ...
  if (controller->telemetry_cb)
    controller->telemetry_cb(controller->telemetry_cb_context,
                             controller->channel, type, value);
}
...
static void dshot_cycle_channel(struct dshot_controller *controller) {
  ...
  pio_sm_init(controller->pio, controller->sm, dshot_pio_prog_offset,
              &controller->c);
...
void dshot_loop_async_complete(struct dshot_controller *controller) {
  ...
  if (absolute_time_diff_us(controller->command_last_time,
                            get_absolute_time()) > DSHOT_IDLE_THRESHOLD) {
